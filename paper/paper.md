...
$R^2$ 0.964; marginal screening admits only $\{ab, (ab)^2, d, e\}$,
...
and can be re-run, rather than on claims. End to end at nominal FDR 0.10, over 200 repeated
trials with a true signal the empirical FDR was 0.0000 with power 1.000
and no fit fell back to unvetted output, and 60 trials with no signal at
all selected nothing. At the selector level,
...
the honesty flag does so on every fit. A separate 315-fit study, included in the repository
