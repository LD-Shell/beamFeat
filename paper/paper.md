...
exposes no `random_state`;its noise-injection feature screen draws decoy
features through the global NumPy generator, and the one seed it sets internally
overrides any seed a caller provides. Six runs on one identical split
...
criteria, and directed development, using Anthropic's Claude as an AI
assistant for implementation and drafting. Every
